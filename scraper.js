        ...
        const pageProperties = propertiesData
          ...
          .map((prop) => {
            ...
            return {
              ...
              brokers:
                prop.listBroker
                  ?.map((b) => `${b.name} ${b.license}`)
                  .join(", ") ?? null,
              contactPerson: prop.associate?.name ?? null,
              office: prop.associate?.officeName ?? null,
              dimensionsLand: prop.dimensionLand ?? null,
              m2Total: prop.dimensionTotalBuilt ?? null,
              m2Cover: prop.dimensionCovered ?? null,
              ambientes: prop.totalRooms > 0 ? prop.totalRooms : null,
              baños: prop.bathrooms > 0 ? prop.bathrooms : null,
